import { ArrowDown, Bell, Game, MenuBars, Message, OfficialLogo, QC, USDG } from "@/public/icons";
...
const Header = () => {
  ...
  return (
    <>
      <div className="bg-primary-200 small-border-b xl:border-b-primary-150 border-b-black">
        <header className="hidden w-full xl:flex xl:items-center xl:justify-between container mx-auto py-6">
          <div>
            ...
            <div className="flex items-center justify-center gap-1">
              ...
              <div className="text-primary-300 text-sm pt-0.5 font-Poppins">Official Page</div>
            </div>
          </div>
          <div className="flex items-center gap-12">
            <div className="flex items-center gap-7">
              <Button variant={variantTypes.outline} px="px-5" text="Create Challenge" onClick={toggleChallenge} />
            ...
            {currentUser.email ?
              <Image priority={true} height={75} width={79} src={Profile} alt="profile" className="cursor-pointer" />
            :
              <div className="flex items-center gap-4">
                <Button onClick={toggleSignup} px="px-7" text="SIGN UP" />
                <Button onClick={toggleLogin} variant={variantTypes.outline} text="SIGN IN" />
              </div>
            }
          </div>
        </header>
        <header className="flex justify-between items-center xl:hidden bg-primary-50 px-5 py-4">
          <div className="flex items-center gap-6">
            ...
            <Image priority={true} height={51.31} width={45} src={PoolLogo} alt="pool logo" />
            <div>
              <h1 className="text-xl font-bold text-white">BITPOOL</h1>
              <div className="flex items-center justify-center gap-1">
                <OfficialLogo size="15" />
                <div className="text-primary-300 ten pt-0.5 font-Poppins">Official Page</div>
              </div>
            </div>
          </div>
          {currentUser.email ? (
            <div className="flex items-center">
              <Link href="/wallet">
                <div className="cursor-pointe px-2 py-3 flex items-center gap-3 bg-primary-950 h-8 rounded-l">
                  ...
                  <div className="font-medium lg:text-base ten text-white font-Poppins">33</div>
                </div>
              </Link>
      ...
      <Modal key={0} Body={Login} isOpen={isOpenLogin} close={toggleLogin} isVoid={1} />
      <Modal key={1} Body={Signup} isOpen={isOpenSignup} close={toggleSignup} isVoid={2} />
      <Modal key={2} Body={NoChallenge} isOpen={isChallengeOpen} close={toggleChallenge} isVoid={3} />
    </>
  );
};
